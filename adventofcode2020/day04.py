...
def pay_attention(passport):
    ...
    if passport["hgt"][-2:] == "cm":
        ...
        return False
    if passport["hcl"][0] == '#':
        if not int(passport["hcl"][1:], 16):
            return False
    else:
        return False
    if passport["ecl"] not in eye_colors:
        return False
    if len(passport["pid"]) != 9 or passport["pid"].isnumeric() == False:
        return False
    return True    
...
if __name__ == "__main__":
    ...
    for x in range(len(passports)):
        valid_count += validate(ast.literal_eval(str(passports[x])))
        if validate(ast.literal_eval(str(passports[x]))):
            real_id += pay_attention(ast.literal_eval(str(passports[x])))

    answer = 'The number of valid passports: {}\nAfter paying attention, these are the real ones: {}'.format(valid_count, real_id)
