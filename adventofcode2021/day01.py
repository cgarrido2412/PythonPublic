...
def part_one(some_list):
    ...
    for x in range(len(some_list)):
        if x == 0:
            ...
            if some_list[x-1] < some_list[x]:
                increased += 1
            else:
                pass
    return increased

def part_two(same_list):
    three_measures = []
    for x in range(len(same_list)):
        if x == 0 or x == len(same_list)-1:
            pass
        else:
            previous = same_list[x-1]
            current = same_list[x]
            next = same_list[x+1]
            total = previous + current + next
            three_measures.append(total)
    increased = part_one(three_measures)
    return increased
