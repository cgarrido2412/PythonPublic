...
with open(file) as working_file:
    ...
    while center_of_mass:
        length = len(center_of_mass)
        
        for kanye in range(length):
            count += number
            
            if center_of_mass[kanye] in object_dictionary:
                
                for west in object_dictionary[center_of_mass[kanye]]:
                    center_of_mass.append(west)

        for kanye in range(length):
            #The pop() method removes the item at the given index from the list and returns the removed item.
            center_of_mass.pop(0) #list.pop(index) 
