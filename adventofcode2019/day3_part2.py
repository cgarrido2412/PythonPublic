...
def nailed_it(movement):
    full_coordinate_dict = {}
    x = 0
    y = 0
    steps = 0
    ...
    for move in movement:
        ...
        for _ in range(0, displacement):
            x += x_movement
            y += y_movement

            steps += 1

            if (x,y) not in full_coordinate_dict:
                full_coordinate_dict[(x,y)] = steps

    return full_coordinate_dict

line_1 = nailed_it(my_move_directions_1)
line_2 = nailed_it(my_move_directions_2)
...
def get_shortest():
    ...
    for intersection in intersections:
        ...
        length_from_zero.append(dist)
		
    return min(length_from_zero)
